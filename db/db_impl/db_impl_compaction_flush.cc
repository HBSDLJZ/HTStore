...
#include "db/parallel_hash_map.h"
#include "db/radix_tree.h"
    ...
    *made_progress = true;
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      for (size_t i = 0; i < c->num_input_files(l); i++) {
        ...
        Arena arena;
        InternalIterator* it = f->fd.table_reader->NewIterator(ReadOptions(false, false), nullptr, &arena, false, TableReaderCaller::kSSTFileReader);
        it->SeekToFirst();
        while(it->Valid()) {
          const Slice& key = it->key();
          int latest_level;
          auto get_value = [&latest_level](int8_t& v) { latest_level=v; };
          bool existsInDram = hash_map.if_contains(key.ToString(true), get_value);
          if (existsInDram) {
            continue;
          }
          if (pop.root()->kv->find(key.ToString(true)) != pop.root()->kv->end()) {
            latest_level = pop.root()->kv->find(key.ToString(true))->value();
            if (latest_level >= c->level(l) + 2) {
              pop.root()->kv->insert_or_assign(key.ToString(true), c->output_level() + 2);
              // printf("move ok.\n");
            }
          }
          it->Next();
        }
        if (c->output_level() > totalLevel) {
          totalLevel = c->output_level();
        }
