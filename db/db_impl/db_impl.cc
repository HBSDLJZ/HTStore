...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!skip_memtable) {
    ...
    if (get_impl_options.get_value) {
      // 假设在mem中
      if (latest_level == 0 && sv->mem->Get(lkey, get_impl_options.value->GetSelf(), timestamp, &s,
      ...
      } else if (latest_level == 1 && (s.ok() || s.IsMergeInProgress()) &&
      ...
      if (latest_level == 0 && sv->mem->Get(lkey, /*value*/ nullptr, /*timestamp=*/nullptr, &s,
                       &merge_context, &max_covering_tombstone_seq,
  ...
  // dram没有找到则去disk找
  if (!done) {
    auto ptr = pop.root()->kv->find(key.ToString(true));
    if (ptr != pop.root()->kv->end()) {
