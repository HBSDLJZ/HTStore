...
// pmempool create obj --layout="radix_tree_layout" --size 247G /pmemfs0/radix_tree_pool.obj
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  pop = pool<root>::open(RADIX_TREE_POOL_PATH, "radix_tree_layout");
  persistent_ptr<radix_tree_type>& kv = pop.root()->kv;
  if (kv == nullptr) {
    transaction::run(pop, [&] {
      kv = make_persistent<radix_tree_type>();
    });
  }
