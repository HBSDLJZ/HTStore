...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    // 1. 如果在内存的hash表找到 则过滤走；
    // 2. 如果在NVM中的radix tree找到 判断是不是当前start_lvl；
    bool should_filter = false;
    int start_lvl = sub_compact->compaction->start_level();
    ...
    if (existsInDram) {
      ...
      if (pop.root()->kv->find(key.ToString(true)) != pop.root()->kv->end()) {
        ...
        if (latest_level >= start_lvl + 2) {
          pop.root()->kv->insert_or_assign(key.ToString(true), out_lvl + 2);
          //printf("compaction ok.\n");
        } else {
          should_filter = true;
        }
      }
    }
    if (out_lvl > totalLevel) {
      totalLevel = out_lvl;
    }
    if (! should_filter) {
