...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
                  ...
                  bool* is_blob, bool do_merge, unsigned int hit_level) {
  ...
  // pop.root()->kv->find(ikey.ToString(true)) != pop.root()->kv->end()
  while (f != nullptr) {
    // 减少table cache带来的io
    if (fp.GetHitFileLevel() != hit_level) {
      continue;
    }
...
bool Version::IsFilterSkipped(int level, bool is_file_last_in_level) {
  ...
  // level1不需要检查bloom filter
  return level >= 1 || (cfd_->ioptions()->optimize_filters_for_hits &&
