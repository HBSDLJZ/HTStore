...
bool MemTable::Add(SequenceNumber s, ValueType type,
  ...
  // transaction::run(pop, [&] {
  //   pop.root()->kv->insert_or_assign(key.ToString(true), 0);
  // });
  hash_map.insert_or_assign(key.ToString(true), 0);
  // int8_t level = 0;
  // auto put_val = [level](int8_t& v){ v= level;};
  // hash_map.try_emplace_l(key.ToString(true), put_val, level);
